...
### Added
- **`experiments.rq1.directive_compliance` and `DirectiveCompliance` — the split that turns the RQ1 negative result into a mechanism.** `action_agreement` says *whether* B's actions track the pose; this says *why not*, by cutting the same rotation handoffs into two independent links.
  - **`directive_agreement`** — does A's stated turn direction match `oracle_action`? **0.5 is a sender writing a fluent instruction that carries no information about which way to turn**, and that is what every 14B arm reads (E3 C0 0.514, E3 C3 0.435, R1 C0 0.468, R2 `nl` 0.368) against the 32B sender's 0.703.
  - **`obedience`** — does B do what it was told? 0.76–0.97 everywhere except `grid` (0.501). Where obedience is high, `receiver_agreement` is pinned to `directive_agreement` — equal to within noise in every arm — so **the bottleneck is the sender, which a receiver-only agreement number cannot show**.
  - **`coverage`** — the share of a condition's handoffs carrying a direction word, and the field that makes a severed directive legible: R1's 40-token prefix cap takes it from 52.3 % to 0.2 %. **The agreement fields are NaN, never 0.0, where nothing was said** — a sender that did not speak is not a sender that was wrong.
  - **Direction words are read from the DELIVERED message, not the raw one**, because that is what B saw; a channel that cuts the directive must show up as coverage collapsing rather than as a spuriously wrong sender. `ROT+` is counterclockwise (`apply_macro_action` adds angular velocity), which pins the mapping, and the pattern lists the longest alternative first so `counterclockwise` is never matched as `clockwise` with the negation dropped. The last direction named wins, not the first: the messages state the problem before the instruction.
  - **Public, and persisted as `directive_compliance.csv`** beside `action_agreement.csv`, for the same reason — it reads only the recorded message, pose and action, so a frozen result can be re-derived without re-fitting a probe. Per-serialisation reads (the R2 arm) come from slicing the record list; no extra code.
  - **`RQ1Result` gains a `directive_compliance` field.** An analysis-output schema change, not a `RunManifest`/`ExperimentConfig` one: existing frozen corpora regenerate cleanly and gain the split, and all four returned datasets have been re-analysed.

### Fixed
- **`LLMClient.health_check` sized its smoke ping at a hard-coded `max_tokens=16`, which cannot fit a Qwen3 thinking trace — R4 (job 247917) burned an eight-hour reservation proving it.** The job reached a healthy endpoint (vLLM answered `GET /v1/models` and `POST /v1/chat/completions` with 200 OK) and then aborted before the first episode with `ServingError: no healthy endpoint`, because `--thinking` raises the *config's* budget to 2,048 while the ping ignored the config entirely. The check failed the very configuration it was meant to certify, and the error text ("raise `max_tokens` on the serving config") misdirected, since raising the config would not have moved the ping.
  - **The ping is now sized to the configuration it certifies:** the configured budget when `chat_template_kwargs["enable_thinking"]` is set, the cheap 16-token ping otherwise. Both branches are pinned on the wire by `test_health_check_pings_at_the_thinking_budget_when_thinking_is_on` and `test_health_check_keeps_the_cheap_ping_without_thinking`. R4 is re-submittable unchanged.

- **`test_no_committed_manifest_records_a_dirty_tree` (`tests/unit/test_manifest.py`) — a guard for a four-session recurring offender.** Both RQ3a manifests have carried `git_dirty: true` since they were first frozen, because analysis runs happen *before* the commit and the re-freeze that clears them is easy to forget. A manifest recording a dirty tree does not identify the code that produced it, so its `git_sha` is decorative.
  - **Skips on a dirty tree, bites in CI.** The check cannot be satisfied in principle while edits are outstanding — clearing a stale flag means re-running the driver on a clean tree — so it is evaluable exactly where it matters. The failure message names the offending files and says the fix is a re-run, never an edit to the field.
- **`runs/af50c7c12d65540f-calibration/` — the E7 gate calibration, on the four-condition corpus.** `fail` AUROC **0.906** (threshold 0.977) and `cosine` **0.766** (threshold −0.647) at a 0.200 firing-rate budget over 3,419 handoffs, against 0.754 / **0.569** on the C0-only G1 corpus. `cosine` is the probe-independent statistic the circularity objection turns on, and 0.569 was barely above chance; this is the first calibration it is usable from. Supplies RQ3b's imported threshold and, under amendment A5, the RQ3a transfer arm's refit statistic.
  - **`calibration.json` is promoted into the frozen run directory** (`runs/rq1/af50c7c12d65540f/calibration.json`), following the pattern of the other two frozen calibrations. The `*-calibration/` directory itself is gitignored — it carries the joblib probes — so without the promoted copy the threshold R5 imports would exist only on the laptop that fitted it. The cluster still refits the joblib (`preceptx-calibrate`), and the committed copy is what that refit is diffed against.
- **`experiments.rq1.action_agreement` and `ActionAgreement` — the per-condition form of G3's correctness limb.** The gate pools the cell; the pooled number is what hid the E3 mechanism.
  - **Same instrument, per condition.** Agreement with `sim.feasibility.oracle_action` against a within-episode permutation null on B's own actions, 2,000 permutations, seeded per condition. Reports `agreement`, `null_mean`, `null_p95`, `p_value`, plus three supporting measures a bare agreement number cannot supply.
  - **`rotation_direction_agreement` is the tie-free read.** Exact-action agreement is harsh near 90°, where both rotation directions are near ties; restricted to handoffs where B rotated *and* the oracle wanted a rotation, it asks only "which way", and 0.5 is a coin flip.
  - **`rotation_flip_rate` is the oscillation measure** — the share of consecutive rotation pairs within an episode that reverse direction, averaged over episodes. **NaN, never 0.0, when no episode holds two rotations to compare**: a 0.0 on a condition that barely rotates would read as perfect stability.
  - **Deliberately a separate function from `pilot.g3_correctness`.** That function produced a verdict of record on 2026-08-29; re-shaping it to take a condition subset would edit an instrument after it had been read. The duplication is the point.
  - **Persisted as `action_agreement.csv`** beside `rq1.json`, and reads only the recorded pose and action — so a frozen result can be re-derived without re-fitting a probe, exactly as `signal_decomposition` can.
  - **First reading (`af50c7c12d65540f`):** C0 0.330 vs null 0.331 (*p* = 0.069), C1 0.046 (*p* = 0.074), C3 0.261 vs 0.342 (*p* = 1.000), **C4 0.399 vs 0.388 (*p* = 0.004, PASS)**. Only the lossy channel produces a receiver that acts on the pose above chance.
- **`--c1-max-tokens`, `--c3-window-rows` and `--c4-dropout` on every sweep driver.** `ChannelConfig` existed but was unreachable from a shell, so **every dataset recorded to date ran at its defaults (8 / 2 / 0.4)** and the declared post-hoc length control (PREREGISTRATION §8b A2) had no knob to turn.
  - **Only flags actually given are passed**, so an untouched axis keeps its default and hashes byte-identically to every prior dataset — pinned by a regression test.
  - **`channel` is inside `sweep_hash`**, so a C1 run at a different cap writes its own dataset and cannot append into the real C1's directory. Tested both ways.
  - **`--dry-run` prints a non-default channel block**, so a mis-set cap is caught before the `qsub` rather than after the episodes are paid for.
- **`PREREGISTRATION.md` §8a/§8b/§8c — the amendment protocol, register, and RQ3b's pre-declared direction.** §8a names what is immutable (evaluated thresholds including "hardest" = C4, *Y*/*V* for confirmatory contrasts, the spent attempt ledger, the circularity guard) against what is amendable (headline order, mechanism-arbitrating cells, secondary analyses, framing), and binds every amendment to four conditions: declared and dated before the run it governs, decision rule stated in advance including how it could disappoint, labelled post-hoc and never pooled into a gate or a family-wise correction, recorded in both §8b and the design log. Opens with A1 (headline order), A2 (the length-holding content swap), A3 (RQ3b runs) and A4 (proposed, blocked on a design decision).
  - **§8c names RQ3b's instrument, not only its direction.** `--statistic cosine`, declared before the `qsub` as §8a requires: it is the probe-independent statistic, so a *positive* result could not be dismissed as the probe grading its own homework, and it is gateable only from the E3 calibration (AUROC 0.766 against 0.569 on the C0-only corpus). Its disappointment branch is stated — an observed firing rate far from the calibrated 0.200 makes the null uninformative rather than supportive.

### Changed
- **The E3 verdict of record is frozen and the E3 ledger is closed (`runs/rq1/af50c7c12d65540f/`).** The rung-2 re-gate returned **`fallback`** — G1 PASS 0.800, G2 FAIL −0.250 (CPVI gap −0.015), G3 groundedness PASS 1.000, G3 correctness FAIL 0.242 against a null 95th percentile of 0.257. Manifest, summary, gate report (`pilot.json`/`pilot.md`) and the full reading committed; `lineage.csv` closed at `gate-verdict`; `PREREGISTRATION.md` §6, `docs/EXPERIMENTS.md` (status board, results log, freeze register) and the design log all record it. **No attempt 3 and no second rung-2 attempt**; F0 will not fire and E4 will not run.
- **RQ2's estimands re-read on the four-condition corpus (`runs/af50c7c12d65540f-rq2/`), and H4 flips.** The G1 confirmation was C0-only, so H4 had no contrast and returned +0.052 [−0.100, 0.201]. All three statistics now separate from their shuffled nulls — `info` +0.275 [0.150, 0.384], `fail` −0.275 [−0.379, −0.155], `cosine` +0.269 [0.147, 0.371] — and `fail` separates realised failure at AUROC **0.906**, up from 0.754. Encoder invariance holds (ρ 0.816, `label_ranking_invariant: true`); the declared rule selects `y_terminal_success` as RQ3b's gate target and leaves RQ1's frozen `y_binary_progress` untouched.
- **`docs/myriad.md` §9 rewritten as the end-to-end run plan (R1–R6) now that the gate track is closed.** Every remaining job with its command, expected dataset and sweep hash diffed from a live dry run, call-count upper bound and the offline steps that gate it; an order-of-operations table; the per-run close-out recipe; and the post-commit RQ3a re-freeze. **Two corrections to what §9b used to say:** the A3 thinking probe's declared seed-pairing against A2 no longer holds anything constant — A2 ran on the T load and the task is now the bar — so it re-pairs against the E3 cell's C0 arm; and every run uses `DRIVER=preceptx-rq1`/`preceptx-rq3b`, never `preceptx-pilot`, which would emit a gate verdict there is no ledger left to spend.
- **`RESEARCH_ROADMAP.md` phase table corrected against the repository.** Phases 5 and 6 were marked "not built" and both shipped; Phase 1 was marked "built, not run" and has closed `fallback`; Phase 4's analysis driver was marked "not built" and has run. Phase 3's milestone is restated as *H1 rejected with a mechanism, H2 supported*, and the "honest read" paragraph is rewritten: the constraint is no longer engineering, it is write-up throughput.
- **`docs/EXPERIMENTS.md` status board and freeze register corrected against the repository.** E6 was marked "not run" and has run; E11 was marked "not run" and its transfer regime is frozen; E4's precondition has failed, so it is marked "will not run" rather than "blocked". F0 is marked "will not fire"; F1 is replaced by **F1′ RQ1 (negative)**, the absent gradient with its mechanism; F2 and F4's transfer half are marked ready-to-freeze / frozen.
- **Amendment A5 executed: the RQ3a transfer arm is re-frozen against the E3 re-gate calibration (`runs/rq3a/{traceelephant,who_and_when}/`).** One calibration substrate now backs the laptop's transfer arms and the cluster's R6 judge arm, replacing the G1 confirmation's `86ecbbdf35322dc3` (`fail` AUROC 0.754) with `af50c7c12d65540f` (0.906).
  - **The arm got partly worse, and A5 pre-declared that this is reported rather than reverted.** TraceElephant agent accuracy **0.525 [0.432, 0.610] → 0.492 [0.407, 0.585]**, while every rank-sensitive measure improved (step 0.110 → 0.127, top-k 0.407 → 0.432, MRR 0.315 → 0.331). The headline survives at its new value: 0.492 [0.407, 0.585] still excludes schema validity 0.263 [0.186, 0.347] and mean cosine 0.254 [0.186, 0.339]. Who&When 0.333 → **0.287 [0.220, 0.367]** against mean cosine's 0.367 — a tie before, a nominal loss on overlapping intervals now.
  - **Both manifests re-run on a clean tree and stamp `git_dirty: false`**, clearing a flag stale for four sessions. They carry two different clean SHAs because the first run's tracked output dirties the tree for the second; that is the guard working, and a single-SHA pair is impossible by construction.
  - **`docs/EXPERIMENTS.md` E11, `RESEARCH_ROADMAP.md` phase 6, `PREREGISTRATION.md` §8b A5 and `docs/myriad.md` §9.0/§9.3** all record the new values and the pre-A5 ones beside them.


### Fixed
- **Four Myriad jobs (244519, 244520, 244522, 244523) lost their GPU allocations to two serving-isolation defects, not to anything about the experiments.** R1, R2 and R4 measured nothing; R3 produced 20 complete Parquet parts that no manifest can turn into a result. Both defects are in the jobscripts, and none of the four dataset or sweep hashes moves — R1 `1f0d58944caa7fc3`/`7b65f40f633311a9`, R2 `a86599bae274eac1`/`9d09bbcece5978f8`, R3 `86b89727699c88fd`/`64e8181c8c95b825`, R4 `0ea4878b6e97f59f`/`a9940b95df220708`, all re-diffed from live dry runs after the change.
  - **`serve_env.json` was one shared file for the whole cluster (`_common.sh`, `pilot.sh`).** `$HOME/Scratch` is mounted by every node, so a single `runs/serve_env.json` was written by every job at once. `write_serve_env` also wrote it *in place*: `cat >"$out"` truncates at redirection setup and the heredoc's `import vllm` / `import torch` substitutions then hold the file at zero bytes for several seconds. 244523 was killed inside that window on node-l00a-003 at 23:21Z, and the zero-byte file outlived the job — 21 minutes later, on node-u00a-001, 244522 finished all 20 parts, read it in `build_sweep_manifest`, and died on `ManifestError: not valid JSON`. **244523 is the only possible culprit**: 244522 wrote a *valid* capture at 23:08:10Z (echoed in its own log), and 244519/244520 were both already dead by 22:58Z.
    - **The capture is now job-scoped** — `runs/serve_env.<JOB_ID>.json`, falling back to `local-$$` off SGE — so two jobs cannot name the same file. The default lives only in `serve_env_path`, and `pilot.sh` exports the resolved value as `SERVE_ENV_PATH` so `serve.sh`'s own call returns the identical string; the two scripts previously each spelled the default out and could have drifted apart silently.
    - **The write stages to `$out.$$.tmp` and `mv`s it**, so a reader sees either the previous capture or the complete new one — never a half. A kill mid-write now leaves the prior capture intact instead of a zero-byte file.
  - **The readiness probe could not tell our server from a stranger's (`pilot.sh`).** The loop polls an address and takes any 200. All three of 244519/244520/244523 landed on node-l00a-003 beside **another tenant's vLLM serving `Devstral-Small-2-24B-Instruct-2512`** on the fixed `:8000`, reported `endpoint live after 1s`, and burned their queue wait reaching a driver health check that — correctly — refused the wrong model. *No repo path can serve Devstral*: the endpoint was never ours, and `TIER` had resolved to `qwen14b` in all three.
    - **The port is claimed before anything is launched.** A `curl` pre-flight refuses to start when the port already answers, naming the conflict and the `-v PORT=<n>` fix; proving the port unheld first is what makes a later 200 attributable to our own server. A listener bound but not yet answering still slips past, and is then caught loudly by the existing `kill -0` check when our vLLM fails to bind.
    - **The default port is offset by the job id** (`8000 + JOB_ID % 1000`), so our own concurrent jobs cannot collide by construction. Overridable with `-v PORT=<n>`.
  - **The serving sidecar is read at start-up, not after the episodes (`experiments/cli.py`).** It was first touched in `build_sweep_manifest`, which runs once every episode is paid for — which is how 244522 lost 30 minutes of A100 episodes to a check that costs nothing at second zero. `_prepare` now reads and discards it, the same instinct as `pilot.sh` warming the embedding encoder before any GPU time is spent. **An unreadable capture stays an error and is never regenerated at manifest time**: reconstructing it in the driver, on a different node, would stamp a manifest with provenance describing a server that did not serve the episodes.
  - **`fetch.sh` matched the literal `serve_env.json`**, which under per-job naming matches nothing; globbed to `serve_env*.json` so the server-side stack does not silently stay on the cluster.
  - **The captures are gitignored (`runs/serve_env*.json`, `runs/serve_env*.tmp`).** Untracked *and* unignored, they put the cluster checkout permanently in `git status --porcelain` — which `manifest.git_dirty` reads verbatim, so every manifest written on Myriad would have recorded `git_dirty: true`, the same provenance failure the RQ3a artefacts were re-frozen twice to clear. Nothing is lost: `build_sweep_manifest` embeds the values and a digest of the bytes into `manifest.json`.
  - **Seven regression tests**, each pinning a defect that actually happened and each verified to fail against the pre-fix scripts: two jobs never share a capture path; a concurrent reader never observes a truncated capture; a killed write leaves the previous capture intact; an occupied port is refused before launch with a diagnostic naming the fix; a free port proceeds and two job ids resolve to different ports; an unreadable sidecar fails before the episodes; a valid one passes the pre-flight.
  - **R3's 20 Parquet parts are diagnostic data, not a run of record**, and R1–R4 all need resubmission — the three that never reached their sweeps because they measured another tenant's model, and R3 because a dataset without a manifest is not a result.
- **`action_agreement` drew every condition's permutation null from one RNG stream (`experiments/rq1.py`).** `default_rng(seed)` was opened outside the condition loop, so each condition consumed whatever stream position its predecessors left and its null depended on **which other conditions happened to be in the dataset**. The same arm scored on a four-condition grid and a two-condition grid reported different *p*-values — and **A2's decision rule reads R1's two-condition agreement limb directly against E3's four-condition C4 value**, so the defect broke exactly the comparison the amendment turns on. Now seeded inside the loop on the condition's fixed index in `CONDITION_ORDER`, which is invariant to the grid; pinned by `test_action_agreement_null_is_invariant_to_the_rest_of_the_grid`.
- **The permutation *p*-value was Monte-Carlo-limited at `n_perm = 200` (`experiments/rq1.py`).** Exposed by the seeding fix, which moved E3's C4 *p* from 0.010 to 0.015 — across the Bonferroni line (0.05/4 = 0.0125), where the 200-draw estimator's standard error is ≈ 0.005, so the corrected claim was never resolvable in either direction. The default is now **2,000**, matching `n_boot`: *p* converges to 0.0040 at 2,000 draws and 0.0060 at 20,000, against 0.0149 at 200. **A precision change, not a re-analysis** — the estimand is the exact permutation *p* and only its Monte-Carlo error moves, and the converged value supports the reading already on the record rather than rescuing one that had failed. Deterministic fields (agreement, rotation direction, flip rate) are unchanged. E3's frozen `runs/af50c7c12d65540f-report/{rq1.json,action_agreement.csv}` and `runs/rq1/af50c7c12d65540f/README.md` are re-derived, and the *p* = 0.010 quoted in `docs/EXPERIMENTS.md`, `PREREGISTRATION.md` and the design log now reads 0.004.
- **`--transfer` accepted a directory it could not load a statistic out of (`experiments/cli.py`, DSE-024 follow-up).** `_transfer_config` validated `calibration.json` but not the probe beside it, and `transfer_scores` degrades a missing joblib to `unavailable`. The frozen `runs/rq1/8902072e1f47b6de/` carries the report and *not* the joblib — trained probes are gitignored — so the `docs/myriad.md` §9c judge command, which named exactly that directory, would have spent the 8h GPU reservation and emitted `cpvi_transfer: unavailable`: the one row §9c exists to protect. `_transfer_config` now checks the persisted statistic too and raises naming the `*-calibration` directory and the `preceptx-calibrate` command that produces it. §9c documents the two offline steps (fetch the dataset, calibrate) ahead of the `qsub`.
- **The localisation table had a residual trace bucket with no counter (`experiments/rq3a.py`, DSE-024 follow-up).** `evaluate` skipped traces with no decisive-step annotation and traces a method did not score, counting neither, so TraceElephant's rows read `scored 220 / evaluated 118 / off-boundary 100` with two traces unaccounted. `LocalisationMetrics.n_traces_not_evaluable` closes it: `scored = evaluated + off_boundary + not_evaluable` now holds on every row, and the TraceElephant residual is identified as two traces carrying no decisive-step annotation. The metrics themselves are unchanged; this is bookkeeping the artefact previously required a footnote to explain.
- **Who&When Pro's 73.9% was read as a same-protocol demonstration of a substrate effect (`docs/rq3a_baselines.md`, `docs/methodology.md`, `docs/experiment_design_log.md`, DSE-047 follow-up).** Re-verified against the paper's HTML full text: 57.5% / 73.9% is confirmed as the Qwen3.5-122B all-at-once **text-modality** cell of Table 4 (the same model reaches 70.4% and 74.9% agent accuracy on image and video, which a text-only benchmark cannot be compared to) — but that paper reports **no** original-Who&When performance row. The 14.2% is 2025 models and the 73.9% is a 2026 model, so the five-fold jump is *suggestive* of a substrate effect, not a controlled demonstration of one. All three documents now say so, and note that frontier models still scored below 10% on Who&When step attribution in September 2025, which is what makes model capability an implausible sole explanation.
- **`runs/rq1/8902072e1f47b6de/README.md` said the G1 confirmation is "3x the episodes".** It is 60 against A2's 24 — 2.5x — and only 40 of the 60 fall in the two gated difficulties, the other 20 adding `hard`, which A2 does not have. Corrected in the frozen artefact.
- **The RQ3a transfer regime was unreachable and its default key could never have resolved (`experiments/rq3a.py`, `experiments/cli.py`, DSE-024).** Both CPVI arms - the headline methods of the RQ3a comparison - were `unavailable` in the frozen result, leaving H5 reported on two surface baselines.
  - **`RQ3aConfig.transfer_key` defaulted to `"s_info"`.** That is the paper's notation, not a `Statistic.key`; the real keys are `fail` and `cosine`, and `info` was retired by DSE-061 anyway. No `load_statistic` could ever have resolved it. Nothing caught it because `transfer_dir` was always `None`, so the arm short-circuited before the key was used. The default is now `fail` and the key is routed through `resolve_statistic_key`, so a config or manifest naming the retired `info` finds the survivor instead of reporting a missing statistic.
  - **`preceptx-rq3a --transfer <calibration dir>` wires the arm, and reads the orientation from the report rather than taking it as a flag.** `orientation` is what the calibration measured against realised failure; a hand-entered sign would silently invert every localisation number in the table. A `--transfer` directory with no `calibration.json`, or no entry for the requested key, raises - a wiring error must not degrade into an unavailable row that reads like "not run yet".
  - **The calibrated *threshold* is deliberately not transferred, only the orientation.** A threshold is an operating point on the arena's score distribution; the log corpora are free-text traces from a different distribution, so the cut-point does not carry even where the ordering does. Every localisation metric is rank-based for that reason and the arm makes no pass/fail claim.
- **The refit arm blamed DSE-042 for an absence it could not observe (`experiments/rq3a.py`, DSE-024).** Its reason read "no replay outcome labels for these steps (DSE-042 has not been run on them)", which reads as *pending*. Measured on both corpora, the truth is stronger.
  - **DSE-042's stated fallback is now actually wired**: with no replay labels, `analyse_rq3a` falls back to the corpus's own trace-level outcome, which is what DSE-042 provides for when replay is cut. `refit_scores` takes a `label_source` so a reason can never misattribute which labels it saw.
  - **The census says the fallback is degenerate on both corpora, with counts.** TraceElephant: 44 failed, 0 succeeded, 176 unlabelled of 220 traces - only its 44 SWE-bench traces carry a `tests_status` and all 44 failed; the other 176 ship truncated at the mistake step, so their `ground_truth` cannot be matched against a final answer. Who&When: 184 failed, 0 succeeded, 0 unlabelled. The refit arm is therefore **undefined without counterfactual replay**, not merely unrun, and DSE-042's "fallback if replay is cut" is measured-degenerate rather than unbuilt.
- **Two RQ1 estimands reported a reassuring number on a single-condition grid instead of refusing (`experiments/rq1.py`, `analysis/stats.py`, DSE-067).** Both surfaced on the reconstructed v9 pilot reports, and both would have been inherited by the G1 confirmation — which is itself C0-only.
  - **`seed_sensitivity` was a self-subtraction, not a degenerate estimate.** It is the per-seed C0-minus-hardest success gap, so with one condition `hardest is C0` and every seed returns exactly 0.0 — output that reads as "the C0→C4 ordering is perfectly seed-stable" at the moment no ordering exists. It now switches estimand to the per-seed success rate, labels it `metric="success_rate"`, records the reason in the artefact, and puts the inference on a Pearson binomial dispersion index.
  - **`binomial_dispersion` returns `None`, never 0.0, on a degenerate null** — fewer than two seeds, or a pooled rate at 0 or 1, where every deviation is zero and the ratio is 0/0. Emitting 0.0 there would rebuild the same false reassurance one level down.
  - **`SeedSensitivity` gains `metric`, `dispersion` and `reason`,** so a reader can tell which quantity was spread across seeds without inferring it from the grid. Its docstring now states that across-seed sensitivity is a *different object* from rerun instability at a fixed seed, which no field here measures. `RQ1Result` is an analysis artefact, not the `RunManifest`/`ExperimentConfig` reproducibility surface, so this re-keys nothing.
  - **`_mixed_model` refuses a rank-deficient design.** H1 and H2 both regress on `C(condition)`; with one condition there is no contrast, and statsmodels answers with `ConvergenceWarning`s and boundary coefficients that land in the artefact looking like estimates. It now returns `converged=False` with empty coefficients and the reason in `mediation_note`.
  - On the two v9 pilots this removes every `ConvergenceWarning` and yields what the zeros were hiding: dispersion 0.64 (A1) and 1.06 (A2) — across-seed variation indistinguishable from binomial noise at a shared rate.
- **`--thinking` could never produce an episode; the `<think>` contract now discriminates solicited from unsolicited traces (`serving/client.py`, DSE-067).** `experiments/cli.py` sets `chat_template_kwargs={"enable_thinking": True}` under the flag, while `chat()` rejected **any** `<think>` unconditionally — so job 233500 raised `ServingError` on its health-check ping, before its first episode, and wrote no Parquet. The two halves were added independently and no test exercised them together.
  - **Unsolicited traces still raise, with the message unchanged.** That guard is what catches a runtime silently ignoring `chat_template_kwargs`; weakening it would let a CoT dump confound every channel condition downstream. It is now conditioned on `enable_thinking` being falsy rather than removed.
  - **Solicited traces are stripped at the boundary, not passed through.** `apply_channel` degrades the *instruction*; a C1 length cap applied to a reasoning dump would truncate the wrong object and measure something other than the channel. Only the text after `</think>` is returned.
  - **An unclosed trace raises rather than falling back to the raw string.** `--thinking` raises `max_tokens` 512 → 2048 precisely because traces are long, so mid-trace truncation is a live outcome; `content.split("</think>")[-1]` would silently return the whole reasoning body as the message. A trace with no close tag is a failed completion, not a message. A closed trace with nothing after it raises too.
  - Four unit tests pin the pair — closed-and-solicited strips, unsolicited raises, unclosed raises, closed-with-empty-remainder raises — because the defect existed only in the *combination* of a valid flag and a valid guard.

### Added
- **G3 gains its declared correctness limb (`experiments/pilot.py`, `sim/feasibility.py`, PREREGISTRATION §7 F0 item).** The grounding limb certifies that a message's numbers are *true*; it cannot certify that the conclusion drawn from them is *right*, and E3 attempt 2 shipped on exactly that gap (grounding 0.999 on a corpus whose modal inference was wrong). The limb is reported beside the grounding limb and never replaces it, so `run_pilot` now returns four `GateResult`s under three gate names.
  - **`sim.feasibility.oracle_action(angle_rad)` is the reference policy**, the per-pose form of the scripted *align onto the lattice point nearest flat, then push east* plan that `certify()` already requires to solve 16/16 jittered starts at every shipped difficulty. `alignment_rotations` was extracted out of `scripted_policy_solves` and both now call it, so the certification limb and the gate limb cannot drift apart. It is a **sufficient** policy, not a unique optimum — position never enters, and near 90° both rotation directions are near ties — so a disagreement means "not the certified plan", not "wrong".
  - **The threshold is a within-episode permutation null on B's own actions, not a chosen number.** Shuffling actions inside an episode preserves that episode's action habits exactly and destroys only their link to the pose. The two failures the limb exists to catch are *invariant* under that permutation — always-push-east (projection blindness) and always-rotate (attempt 2's defect) score exactly the null by construction — so neither passes by being lucky about a base rate. 200 permutations, one-sided 95th percentile; deliberately **not** RD-15's beat-every-permutation criterion, which at 200 permutations is *p* ≤ 0.005 and would make the gate stricter than the hypothesis tests it licenses.
  - **`detail` carries the diagnosis, not just the verdict**: `state_blind_mean`, `excess_over_null_mean`, `null_max`, `p_value`, `oracle_frac_push`, and `rotation_direction_agreement` (0.5 = B turns at random), because a bare agreement number cannot say *why* a limb failed.
  - **A constant reference action returns `assessable=False`**, matching G2's discipline on a flat label: where the oracle never varies, agreement measures habit rather than reading, which is no evidence rather than a failure, and spends no retune.
  - **First reading, on the G1 confirmation `86ecbbdf35322dc3`: FAIL.** Agreement 0.285 against a null 95th percentile of 0.322 (null mean 0.315, *p* = 1.000) — the pair matches the certified plan *less* often than its own shuffled actions do — while grounding on the same corpus reads 0.9998. Supporting: 4 of 60 episodes ever bring the load within 6° of alignment (0.65 % of handoffs), rotation-direction agreement 0.519 on 1,400 rotations, mean |misalignment| 84.5° → 36.3° first-to-last handoff, ROT+ 723 vs ROT- 678.
- **The RQ2 analysis is run on the G1 confirmation for the first time (`runs/86ecbbdf35322dc3-rq2/`, offline, no GPU).** It supplies the Phase-2 freeze evidence and one substantive negative result.
  - **H4 does not hold in its literal form.** The shuffle-corrected Spearman between each runtime statistic and CPVI is indistinguishable from zero — `fail` +0.052 [−0.100, 0.201], `info` −0.052 [−0.201, 0.100], `cosine` −0.007 [−0.123, 0.124] at 200 permutations — while `fail` predicts *realised failure* at AUROC 0.754 and `info` is its exact rank-inverse (0.246 = 1 − 0.754, the DSE-061 identity showing through). The statistic that gates well is not a CPVI proxy. PREREGISTRATION anticipated this shape ("agreement with CPVI is a *reported correlation*, never a calibration target"), so it is reportable rather than fatal, but it narrows what RQ3b may claim: the gate is justified by direct calibration against realised outcomes, never as an approximation to CPVI.
  - **H3 twins rank-agree and do not numerically agree.** Spearman **0.612**, Pearson 0.335, Bland–Altman bias 0.071 with limits [−0.844, 0.986] against a mean CPVI of 0.19. The rank claim is supported; the numeric-equivalence claim is not made.
  - **Y confirmed as `y_binary_progress` on the pre-declared rule.** Corrected CPVI by label: `y_binary_progress` 0.1869 [0.156, 0.218], `y_continuous_displacement` 0.2447 [0.168, 0.321], `y_terminal_success` 0.0585 [0.027, 0.098], `y_discrete_config` 0.1167 and **inadmissible** (minority share 0.027). Selection ran admissibility → encoder-invariance → twin agreement, with corrected effect size only a tie-break inside 0.05, per the rule fixed in `experiments/rq2.py` before any outcome was read.
  - **Encoder sensitivity ran and is clean.** Primary CPVI 0.1858 (`bge-base-en-v1.5`) against 0.1519 (`all-mpnet-base-v2`), ρ = 0.842, `label_ranking_invariant` **true** — the ordering of candidate outcomes is not an artefact of the primary encoder.
- **The G1 confirmation is frozen (`runs/rq1/86ecbbdf35322dc3/`, job 236653, dataset `86ecbbdf35322dc3`).** 60 episodes / 2,597 handoffs / 37 min on one A100-40GB, C0 × easy+medium+hard × seeds 12–31 at `max_steps` 50, prompt v9, `git_sha` 9631852 with the manifest, summary and calibration committed and the Parquet not.
  - **G1 PASS at exactly the pre-registered floor.** Easy C0 **10/20 = 0.500** against *≥ 0.5 at easy difficulty* (PREREGISTRATION §6), Wilson 95% **[0.299, 0.701]**; medium 3/20, hard 1/20, 14/60 overall. Recorded everywhere as a pass on the letter of the declaration, never as demonstrated capability.
  - **G2 unassessed, not failed.** The run is C0 only, so `contrasts` is empty and the mixed model refuses to fit ("only condition C0 is present, so `C(condition)` is rank-deficient"). An unassessable gate spends no retune; the declared E3 cell is the next run.
  - **G3 reported as grounding-limb only, at 0.9998** (2,597/2,597 messages cite numbers, 2,593 perfectly grounded; per-difficulty 1.0000 / 0.9997 / 0.9997). The correctness limb PREREGISTRATION declares at F0 — agreement with the oracle's next action — is **not implemented**: `pilot.py` carries the grounding limb only while an oracle exists in `sim/feasibility.py`. Attempt 2 scored 0.999 grounding while reasoning wrongly, so the number alone does not separate the two corpora.
  - **The measurement primitive replicates on disjoint seeds.** CPVI 0.1853 [0.152, 0.217] against the A2 pilot's 0.1876 [0.115, 0.249]; PVI 0.2695 vs 0.2728; PVI−CPVI gap 0.0842 vs 0.0851; control CPVI −0.0016 vs −0.0024; selectivity 0.1869 vs 0.1900 — every line within ~1%, interval roughly halved, while the matched outcome rate moved 0.417 → 0.325. Shuffled-message audit: observed CPVI above all 20 permutations (null mean −0.0012, max 0.0015, *p* = 0.0476 = the 1/21 floor). Signal decomposition: absent-signal 0.412 [0.357, 0.465], unused-signal 0.265 [0.215, 0.321]. Carried forward as a confound: CPVI's partial Spearman against message length is **0.439**.
- **The next GPU submission is specified and costed in `docs/myriad.md` §9a: the E3 re-gate for G2** — the pre-registered cell C0/C1/C3/C4 × easy/hard × seeds 0–9, which `preceptx-pilot` already encodes. Both candidate hashes are pre-registered in `lineage.csv`: `af50c7c12d65540f` at `--max-steps 50` (8,000 calls, ~50 min; coherent with the configuration G1 was declared over, but a declared deviation from the pre-registered 30/35 budget) and `0ded5641590b44a9` undeviated (5,200 calls, ~37 min). One or the other, not both.
- **An episode-cluster interval on the transferred statistic's home AUROC (`runs/rq1/8902072e1f47b6de/README.md`, `docs/experiment_design_log.md`, DSE-024 follow-up).** The A2 `fail` AUROC of 0.593 was reported as a bare point estimate. Its 95% CI over the pilot's 24 episodes is **[0.444, 0.737]** (2,000 resamples of episodes, matching the clustering `calibrate` cross-fits on), with 10.8% of resamples at or below 0.5 — so the statistic is **not established above chance at home**, and the `+1` orientation the transfer arm multiplies by is the sign of that same underpowered interval. Recorded as a caveat on the pilot rather than on the transfer result: ranking a handful of handoffs within one trace against an annotation and predicting prospective episode failure from a single handoff are different estimands, and the transfer arm carries its own intervals on its own substrate. Re-keying to the G1 confirmation (60 episodes, unseen seeds) is what tightens it.
- **The `cpvi_transfer` / `mean_cosine` tie on Who&When is decomposed (`docs/experiment_design_log.md`, DSE-024 follow-up).** Both score 0.367 agent accuracy there — the same 55 of 150 — which after DSE-061 retired `InfoStatistic` as rank-identical to `FailStatistic` warranted a check. They are near-orthogonal, not identical: they share only 17 of the 55, each is right on 38 the other misses, and their within-trace Spearman correlation is 0.03. On TraceElephant they are *anti*-correlated (within-trace ρ = −0.24, median −0.50) with 10 shared hits out of 68 and 30. Post-hoc diagnostic on the frozen scores; no reported metric changes.
- **`runs/rq1/188a3d556b824e3e/lineage.csv` reconciles the planned and realised D26 hashes.** All three arms were planned in 5b4c0bf against prompt v8 (`8e438b0a…`, `7653131e…`, `d66e67ae…`) and realised at v9 (`9f46e0e3…`, `8902072e…`, `9fe1823c…`), because 5d0b901 replaced the declared slit width with the pose-dependent clearance line before submission. One intended change, three deliberate re-keys — the fingerprint behaving as designed, now stated per row rather than inferable. The pending G1 confirmation row records that its declared `86ecbbdf35322dc3` (sweep `2e201daa64f6c4ac`) was re-verified by dry-run at `dc70c54`, i.e. after PR #74's provenance hardening, so the gate declaration holds.
- **`preceptx-calibrate`, the only producer of two artefacts the rest of the stack already consumed (`experiments/cli.py`, `gate/calibration.py`, DSE-024).** `save_statistic` and `write_report` shipped in DSE-017/DSE-018 and were called nowhere outside a unit test; no calibration existed anywhere on disk. Two consumers were blocked on that: `rq3b.build_gate` imports its threshold from a persisted `CalibrationReport`, and the RQ3a transfer regime loads a persisted `Statistic`. Offline, no GPU, shaped like `preceptx-analyse`.
  - **`statistic_factories(probe)`** hoists the factory list out of `calibrate`, so the report and the joblibs beside it enumerate the same keys by construction. A report naming a key with no statistic next to it would have been a silently unavailable arm.
  - **`fit_statistics`** fits each statistic on the **whole** calibration set and is deliberately distinct from what `calibrate` fits: `calibrate` cross-fits by episode so its AUROC, ECE and threshold are honest held-out numbers, then discards the folds. A statistic that will be *applied* to another substrate wants every labelled handoff behind it, with the report beside it saying how far to trust it.
  - The target stays realised episode failure, never CPVI (R5); this entry point only chooses which dataset supplies the outcomes. The joblibs are gitignored as trained probes; `calibration.json` is a frozen provenance artefact and is not.
- **`docs/rq3a_baselines.md`: the dated baselines table and the contribution axis (DSE-047).** H5 was anchored to Who&When's 53.5% / 14.2%, both since beaten more than once. Every figure was verified on 29 Aug 2026 against the paper's own abstract or results table, with the one row two reads disagreed on marked for a human check.
  - **Two numbers in `docs/methodology.md` section 2 were secondary-sourced and are corrected.** AgenTracer's margin is "up to 18.18% over Gemini-2.5-Pro and Claude-4-Sonnet", not "roughly an 18% relative margin over frontier reasoning models"; 66.7% / 33.3% is the *dynamic* agentic method specifically (static agentic is 65.9% agent).
  - **Two 2026 results the review predates are added.** Who&When Pro (Liu et al., 2026) reports 57.5% agent / **73.9%** step - on 12,326 trajectories whose failures were *injected* after replaying a successful prefix, so the decisive step is placed by construction and the figure is **not** commensurable with Who&When's 14.2% over naturally occurring errors. That the same all-at-once protocol produces both is the table's own evidence that these accuracies index the substrate at least as much as the method. Long-Horizon Agent Trajectory Attribution (Aug 2026) is tabulated because it is the one corpus in the table that **contains non-failures** (30.3% task-aligned), which is the class both RQ3a corpora lack.
  - **The contribution is restated on prospectivity and cost**, both properties of the construct rather than leaderboard positions, with the measured cost differential in the table: zero model calls per handoff against 3,428 (TraceElephant) and 4,380 (Who&When) upper-bound judge calls for the three published procedures.
  - **The AgentForesight adjacency is narrowed rather than defended.** `methodology.md` section 7 already carried the measure-versus-detector, cost and intervention arguments and stays canonical; the new section adds only what it left implicit - a prefix-reading auditor **cannot be entered as the H2 mediator** on a channel manipulation, because it reads the manipulated and unmanipulated parts of the trajectory together. `docs/abstract.md` no longer reads as claiming prospectivity alone is novel.
- **`OutcomeCensus` on `RQ3aResult` (`experiments/rq3a.py`, DSE-024).** Counts the annotation-free outcome classes a corpus actually supplies, so an unavailable refit row carries a measured reason rather than an assumed one. Carried into `RunManifest.metrics` under `rq3a.outcomes`.
- **`scripts/myriad/fetch.sh`, the one supported way to bring a run home (DSE-067).** The 29 Aug results bundle was assembled by hand and took only `runs/<hash>/*.parquet`, leaving both v9 arms' manifests on the cluster — the one artefact no local work rebuilds, since `preceptx-analyse` reconstructs the analysis (encoder revision, probe config) but not the model revision, the exact command or the serving environment. No argument fetches every run's `manifest.json`, `summary.json` and `serve_env.json`; a dataset hash additionally fetches that run's Parquet and stages it at `runs/<hash>/` where `preceptx-analyse` looks. Parquet is skipped by default so a 96-part dataset does not arrive whenever someone wants a manifest.
- **`preceptx-rq3b` and the H6 causal-gate arms (`experiments/rq3b.py`, `experiments/cli.py`, DSE-025).** DSE-018 shipped the gate and its two score-blind controls; this is the experiment that uses them. RQ1 and RQ2 detect — this asks whether detection becomes enforcement.
  - **`rq3b_sweeps`** derives four sweeps from one ungated base, differing *only* in `gate`, and refuses a base that is already gated (one arm's retry budget would otherwise reach all four). It asserts four distinct dataset hashes before returning: `SweepConfig.gate` is inside `dataset_hash_for`, so pooling an arm would not error — it would quietly average the treatment into its own control. The `off` arm keeps `gate=None` rather than `GateConfig(mode="off")`, which behaves identically in the loop but hashes as every ungated dataset to date, so the no-gate arm can reuse an existing RQ1 dataset instead of paying to re-run it.
  - **`build_gate`** imports the threshold from a persisted `CalibrationReport` and never re-derives it; the statistic is re-fitted on the *calibration* records, not on the arm's own episodes, so the treatment cannot pick the operating point it is about to be judged at. A missing statistic key raises rather than failing open — a gate that cannot score would be recorded as a gated arm while behaving like the ungated one. Only the `active` arm is given a fitted statistic: a control holding one would read, in the artefact, as though it had seen the score it is defined to be blind to.
  - **`analyse_rq3b`** works at the episode level (the unit the intervention resolves at) and tests gate-active against all three controls on both terminal success and steps — six contrasts as **one** Holm family, because H6 is a single hypothesis and correcting per-outcome would smuggle six tests in as two families of three. Reports bootstrap deltas with intervals and Cliff's delta, never bare significance. Requires all four arms and fails loud on a missing one.
  - **`verdict` separates a null from an untestable comparison.** "The gate does not beat its score-blind controls" is a reportable finding about the statistic. Terminal success being *constant across every episode of every arm* is not — it means the grid produced no variance to move, which is the live risk after job 232980's 1/96, and is named as a statement about the grid rather than about the gate. Degeneracy is judged on the primary outcome alone: every gated arm re-prompts where the ungated one does not, so the arms' step counts differ on any real grid and a guard keyed on the (success, steps) pair would have let a wholly floored run through as "H6 NOT SUPPORTED". The verdict also names the correction it applied rather than hardcoding Holm.
  - **The realised firing rate is measured per arm, not assumed.** If `matched_random` does not land within 0.05 of the active gate's, the run logs a warning: the rate-matched control was not matched, and its contrast is weaker evidence than it looks.
  - `write_rq3b` emits `rq3b.json`, the mode and contrast CSVs, `verdict.md`, and outcome-by-mode CI figures. `--dry-run` prints all four arm hashes and states that the cost is four times the printed grid.
- **Frozen artefacts for RQ1 job 232980 (`runs/rq1/188a3d556b824e3e/`).** Manifest, summary, a README that states in its own words that this is a C0-only capability grid and not a gradient or gate result, `lineage.csv` (one row per dataset generation, including the three planned D26 arms, so a reader can see which runs are comparable), and the SHA-256 of the raw-data release asset. The 96 Parquet parts stay out of Git and out of LFS.

- **Prompt v9: the pose-dependent clearance and the gap's usable width in every state form (`sim/load.py`, `sim/arena.py`, `sim/serialise.py`, `agents/prompts.py`, D26).** Job 232980 floored at 1/96 with 97.6% of `E` actions issued at poses that could not fit the slit; 99.9% of messages quoted the angle and 6.6% attempted the projection.
  - **`load.extent_y(angle)`** returns `1.4·|sin θ| + 0.3·|cos θ|` — `BAR_THICK` at 0, `BAR_LEN` at π/2, and neither constant anywhere between. The modal v7 error was substituting `BAR_THICK` for it at a near-vertical pose 4.7× wider than that.
  - **`arena.usable_gap(slit_width, geometry)`** returns `slit_width − 2·wall_radius`, the aperture the walls actually impose: `build_arena` gives every segment a `wall_radius = 0.05` lip, so the free gap is 0.1 narrower than the declared width. It is the single source of truth — `alignment_tolerance()` now derives from it instead of repeating the subtraction. Verified against the simulator, not on paper: the largest angle a slit-centred bar actually threads is 38.0/17.0/10.0° at 1.20/0.80/0.64, which `extent_y ≤ usable_gap` reproduces exactly and `extent_y ≤ slit_width` over-accepts by 6.5/4.5/4.0° — 15%/21%/29% of the poses that looser rule certifies.
  - **`serialise.clearance_line`** puts **both** scalars — `slit_clearance` and `load_extent_y` — in **all three** forms: numeric, grid header and NL. Isomorphism is the reason: withholding either from one form would make that form measure trigonometry rather than representation, which is not the axis the serialisation A/B exists for. The NL form named no aperture at all before this, so it could not have performed the comparison the line invites. It states two scalars, not a verdict — the comparison, the rotate-then-translate ordering, the y-alignment and the two-wall repetition remain the agent's inference.
  - **`serialise.split_grid`** replaces line-counting in the two grid consumers. The grid header was one line (the legend) until v8 made it two, and both `_grid_load_centroid` and the C3 row window silently mis-indexed every raster row when it grew. A raster row is now recognised by its glyph alphabet (`T G # .`), which costs the same and does not need revisiting the next time the header changes.
  - **C3 windowing re-prepends the legend only.** The numeric whitelist (`load=`, `contact=`) and the NL first-sentence rule both drop the clearance already, so keeping it in the grid header would have left the grid arm under a materially weaker C3 than the other two and confounded the condition contrast with the serialisation axis it is crossed against. The legend is selected **by name**, so a header line added later is dropped from C3 until someone deliberately admits it — the same fail-closed rule the numeric whitelist uses.
  - `PROMPT_VERSION` v7 → v9, so v7 (`188a3d556b824e3e`) stays intact as the baseline arm rather than being resumed into. v8 named the span beside the *declared* width and never ran; the planned D26 arms re-key to `9f46e0e34fab81cf` / `8902072e1f47b6de` / `9fe1823c20d33c75`.
- **`--max-steps` on `preceptx-rq1` / `preceptx-pilot` (`experiments/cli.py`).** Broadcasts one budget over the certified per-difficulty `STEP_BUDGETS`. The certificate bounds an *optimal* policy; all 96 episodes of job 232980 saturated their cap and the single success finished at step 28 of 30, so the budget is a live constraint. Omitting the flag keeps the certified budgets — it is not passed as `None`, which would fail the validator instead of falling back to the field's `default_factory`.
- **`--thinking` on `preceptx-rq1` / `preceptx-pilot`.** Enables the Qwen3 reasoning trace via `chat_template_kwargs` and raises `max_tokens` 512 → 2048; at 512 a trace truncates mid-token and the structured action never arrives.

- **`preceptx-rq3a`, the RQ3a driver (`experiments/rq3a_run.py`, `experiments/cli.py`, DSE-064).** DSE-031 shipped the pilot/RQ1/RQ2 entry points and listed "the gate and RQ3 drivers" as out of scope; the gate driver arrived with DSE-018, and this is the other half. Until now `analyse_rq3a` and the loaders, scorers and audits under it — roughly 1,800 lines — could only be reached from a test.
  - **`run_rq3a`** loads one per-step corpus, scores every available method on it and builds the manifest in one call. `judge=None` and `with_mast=False` are *supported modes*, not degraded ones: the judge rows are absent and `cpvi_transfer` keeps its row with a reason, so an offline run is a complete artefact rather than a partial one.
  - **`corpus_digest`** is the log substrate's analogue of `dataset_hash_for`: a sha256 over the loaded records in load order, so it is stable across a re-download or a re-zip and moves the moment an upstream HuggingFace upload is revised. Taken over what the analysis consumes, not over the downloaded bytes.
  - **`RQ3aManifest`** is a third manifest type on its own version counter, beside `RunManifest` and `SweepManifest`. Neither of those describes a run over real logs — one requires an `ExperimentConfig`, the other a simulation fingerprint — and fabricating either would write invented physics into a manifest, the weakening DSE-041 already rejected when it kept `LogHandoffRecord` separate from `HandoffRecord`. Records corpus identity and counts, both primary and MAST, the encoder revision, the judge identity, the substrate label and `serve_env`.
  - **`projected_judge_calls`** is the pre-flight cost: one call per trace for all-at-once, `ceil(log2 n)` for binary search, *n* for step-by-step. Each term is pinned by a test to some judge's real worst case, so the bound is neither exceeded nor loose — the all-no judge is the worst case for step-by-step and the *cheap* branch for binary search, and only the all-yes judge exercises the other half.
  - **The CLI sits between `preceptx-rq1` and `preceptx-rq2`.** It reads a corpus off disk like the latter, but the three judge replications cost model calls, so the endpoint, the Hydra model block and `PRECEPTX_SERVING_SUBSTRATE` are demanded **only under `--judge`**. `--dry-run` prints the corpus counts and the judge's projected cost without constructing a client.
- **`VLLMJudge`, the open-weight annotator for the three Who&When procedures (`experiments/rq3a_run.py`, DSE-065).** ~60 lines over the existing `LLMClient.structured()`, so the procedures stay in `rq3a.py` and no answer parsing leaks into the analysis.
  - **Abstention is decoded, never caught.** The schemas offer `{"step": -1}` and `{"answer": "unsure"}` as reachable answers, and an out-of-range index is also an abstention. A `ServingError` is *not* converted: an abstention is a claim about the trace, an outage is not, and recording one as the other would put endpoint downtime into the results table as judge behaviour.
  - Records `model_name`, `model_revision` and the resolved decoding into `JudgeIdentity`, which already carries the caveat that these are a replication against an open-weight tier and not the published frontier-annotator figures.

- **Plannability and scripted-policy limbs on the feasibility certificate (`sim/feasibility.py`, DSE-060, DSE-063).** A certificate now has to show three things, because run 227886 passed the first and failed the other two invisibly.
  - `assert_plannable` replays a certified path and rejects it if any rotation differs from the free-space quantum by more than 0.5°. The DSE-058 certificates for medium and hard each threaded the channel on a **contact-truncated** rotation (34.68° and 42.49° against a 57.79° free quantum) landing inside the window by arithmetic luck — sound physics, but not a plan an agent could state while being told the quantum is constant.
  - `scripted_policy_solves` runs the obvious plan (rotate onto the nearest passing angle, then push east) from each jittered start and reports the share solved. A\* searches from the *canonical* pose, so a start-pose offset the action set cannot correct is structurally invisible to it — which is how a `y_range` the actuator could not close shipped unnoticed.
  - `certify()` now runs all three limbs and raises `PlannabilityError` rather than returning. Threshold is 1.0: anything less means some start pose cannot be corrected by the action set, which is a property of the task and not of the agent.
  - `replay` and `measure_rotation_step` are the underlying instruments, exported so a diagnosis can use them without re-deriving the physics.
- **`preceptx-analyse`, the offline half of `preceptx-rq1` (`experiments/cli.py`, DSE-062).** `--no-analysis` runs the episodes and stops; `preceptx-analyse --dataset-hash <h>` runs the analysis later from disk. Job 227886 held an A100 for 2h37m running statsmodels, and the analysis is also the only part that can fail *after* the episodes are paid for. No Hydra model resolution and no serving substrate, matching `preceptx-rq2`.
- **`arena.alignment_tolerance()`** — how far off the slit centre a flat bar may sit and still clear the narrowest channel, derived from the tightest aperture and the bar thickness rather than authored, so a change to either moves it automatically.
- **RQ1 job 227886 run evaluation and actuator diagnosis (`docs/rq1_227886_diagnosis.md`).** The
  post-run analysis of the D23 characterisation sweep (dataset `54ed65e6cc9e7d17`, 480 episodes,
  11,052 handoffs, Qwen3-14B). Documentation only - no estimator, simulator or config behaviour
  changes here, and **nothing is applied to job 227886**, which stands as the diagnosed baseline.
  - **The actuator root cause.** `StepConfig.angular_impulse = 0.5` was sized against the **T-load**
    (moment 0.2927, ~34 deg/action, as its own comment still states). DSE-057 swapped in the convex
    bar (moment **0.1708**, **1.71x** lighter to spin), so the same impulse now sweeps a **measured
    49.1 deg +/- 16.9 deg per action** against threading windows of **90 / 44 / 17 deg** for easy /
    medium / hard. On medium one action exceeds the whole tolerance window; on hard by 3x.
    **100% / 69% / 22%** of C0 episodes ever reached a threadable pose. Re-derived targets:
    `angular_impulse` ~ **0.20** for medium, ~ **0.08** for hard. Not applied.
  - **The compounding change.** DSE-058's `hold_orientation = True` correctly closed a real
    degeneracy (up to 114 deg of contact rotation threading the channel with no rotate command) but
    removed the only source of sub-quantum angular adjustment in the same commit. Confirmed in the
    data: `E`/`N`/`S`/`W` produce Delta-angle 0.0000 with sd 0.0000.
  - **C1 is a collapsed policy, not a degraded channel.** `E` on **100%** of all 2,240 C1 handoffs -
    zero rotations, zero N/S - with **21.7 collisions per episode** against C0's 3.2, and action
    entropy **0.000** against 2.03-2.19 elsewhere. C1 is simultaneously the **highest** leakage-
    corrected CPVI (**+0.118**) and the **only** 0/96 arm, so H2's mediator moves opposite to the
    outcome.
  - **The RQ1 contrasts are null under D23's declared estimand.** Every C0-Ck corrected CPVI
    interval straddles zero; the C0-C4 gap is positive in **19/32** seeds (mean +0.027, sd 0.112).
    On the outcome side only C1's Cliff's delta excludes zero (-0.063 [-0.115, -0.021]).
  - **The pooled measurement is unaffected and at its strongest.** Leakage-corrected mean CPVI
    **+0.0895 bits [+0.0753, +0.1036]** against a random-label control of -0.0021 and a permutation
    null of +0.0034, with a **PVI - CPVI gap of +0.111** (54% of apparent message value is state
    echo) - ~2x the three prior datasets, now replicated across four runs and two task geometries.
    Groundedness: **99.98%** of quoted positions within 0.01 units of the true state.
  - **Listening-target CPVI (new analysis, document only).** Scored within each condition so no
    probe can read the condition tag, target = the recipient's next macro-action: C0 **+0.435**,
    C2 **+0.449**, C3 **+0.403**, C4 **+0.462**, C1 **0.000** (single action class, undefined by
    construction; nulls -0.007 to -0.016). **A gate thresholding the state target ranks the
    collapsed arm first; one thresholding the recipient's next action ranks it last.**
  - **Gate calibration on this dataset**, target realised failure, 0.2 budget, out-of-fold under an
    episode group split: `info` **0.870** AUROC / ECE 0.0035, `fail` **0.870** / 0.0055, `cosine`
    **0.619** / 0.000, at n = 11,052 (`ece_reliable: true`, a first). Sliced against the D23
    identity check: `info`/`fail` hold at **0.838** with C1 excluded and 0.66-0.87 within condition,
    so they read message content rather than the channel tag - **the RQ3b premise holds**. But
    `cosine` falls to **0.522, chance**, without C1, so the one statistic that exists to answer the
    circularity objection is the one most dependent on the degenerate arm, and must be reported so.
  - **RQ2 on this dataset (`runs/54ed65e6cc9e7d17-rq2/`).** The D24 rule, fixed in code before any
    of this run's output existed, **re-selects the frozen label**: `y_binary_progress` is the
    recommended RQ3b gate target, chosen on encoder-invariance (0.689 vs 0.585) among the two
    admissible candidates, with encoder rho above the 0.50 re-freeze flag so no re-freeze is
    triggered. `y_discrete_config` carries the **highest** corrected CPVI of the four (**+0.153**
    [+0.132, +0.174]) and `y_terminal_success` is the label anyone would want to gate on, and **both
    are ruled inadmissible on minority share** (1.4% and 3.6% against the 0.10 floor) - the rule
    caught this run's floor effect without being told about it.
  - **H4 as specified is not supported.** Best corrected Spearman against CPVI is **+0.073**
    [+0.007, +0.133] (`info`); `cosine` is **-0.019** [-0.078, +0.035], indistinguishable from zero.
    For `info`/`fail` the **null exceeds the real correlation** (0.179 vs 0.106) - the artefact D24
    built the null to catch. **But `fail` predicts failure at AUROC 0.869**, so the proxy->CPVI link
    fails while the proxy->outcome link, which is the one RQ3b needs, holds.
  - **H3 twin agreement.** Pooled Pearson 0.259, Spearman 0.489, Bland-Altman bias +0.029, limits of
    agreement **[-0.710, +0.769]**, `n_kl_capped = 0`. The limits are +/-0.74 bits on a quantity
    whose mean is 0.09, so the prospective twin is **not** a per-handoff substitute for CPVI and is
    not written up as one; it works at the episode level (AUROC 0.832 pooled, 0.887 across cells).
  - **`InfoStatistic` and `FailStatistic` are rank-identical** (Spearman **-1.000000** on their
    out-of-fold scores; +/-0.474 against `cosine`). Expected - both are probe-backed on the same
    embeddings predicting the same binary label - but the gate has **two** independent statistics,
    not three, and `GateConfig.statistic_key` offers a choice between two of them that does not
    exist.
  - **Known-bad artefact flagged.** `_handoff_model`'s `vc={"episode": "0 + C(episode)"}` expands
    480 episode dummies and does not converge (`|grad|` 4.2-8.3, "MLE may be on the boundary"), so
    **the H1 p-values in `runs/54ed65e6cc9e7d17-report/rq1.json` are not trustworthy** and are
    quoted nowhere; the episode-cluster bootstrap carries every interval reported. Two operational
    notes from the same job: the offline analysis held an A100 for **2h37m** doing CPU-only
    statistics, and every figure was skipped for a missing `viz` extra in the container.
...
### Changed
- **Permutation counts raised 20 → 200 (`experiments/rq1.py`, `experiments/rq2.py`).** A permutation p-value floors at 1/(n+1), so at 20 the strongest statement the RD-15 audit can make — the observed CPVI exceeding *every* permutation — printed as *p* = 0.0476 and read as a marginal pass. Re-run on `86ecbbdf35322dc3` the same outcome now quotes as ***p* = 0.00498**, with mean CPVI 0.1853 against a null mean of −0.0010 and a null max of 0.0020 (173 null-SDs). In RQ2 the null is *averaged* rather than thresholded, so there the gain is variance rather than resolution — a noisy null attenuates every shuffle-corrected correlation, which is the H4 headline — and repeats are dropped inside the null, so 200 costs ~2 min on a 2.6k-handoff cell. `runs/86ecbbdf35322dc3-report/` and `runs/86ecbbdf35322dc3-rq2/` are regenerated at the new count.
- **The E3 step-budget deviation is logged (PREREGISTRATION §6).** The G1 verdict of record ran `--max-steps 50` broadcast to every difficulty against certified budgets of 30/35/35 (`STEP_BUDGETS`, ceil(2.5 × the oracle optimum)), which §2 fixes and §7 reaffirms, and that was never recorded. It is now, and the E3 re-gate runs at 50 as well so one gate does not describe two task parameterisations; D26 measured the budget effect as not significant (*p* = 0.63 / 1.00), and the correctness limb supplies the independent reason extra steps bought nothing here.
- **The RQ3a transfer arm is re-keyed from the A2 pilot to the G1 confirmation, and the frozen results move with it (`runs/rq3a/*/`, `transfer_train_dataset_hash` 8902072e1f47b6de → 86ecbbdf35322dc3).** The re-key was declared unconditionally in `runs/rq1/8902072e1f47b6de/README.md` before the confirmation existed, and was taken as declared — **including where it lowered the headline number**.
  - **At home the re-keyed statistic is unambiguously better.** `fail` AUROC **0.593 → 0.754**, episode-cluster 95% CI [0.444, 0.737] → **[0.638, 0.870]** with no resample at or below 0.5, ECE 0.037 → 0.031, and `ece_reliable` true for the first time (n = 2,597 ≥ 200). The caveat this project carried — "the transferred statistic is not established above chance at home" — is closed by measurement.
  - **On the log corpora it is slightly worse on the headline and better elsewhere.** TraceElephant `cpvi_transfer` agent accuracy 0.576 [0.492, 0.661] → **0.525 [0.432, 0.610]**, step 0.169 → 0.110, MRR 0.376 → 0.315; Who&When agent 0.367 → 0.333, step 0.180 → **0.207**, MRR 0.381 → **0.415**. So held-out home AUROC does not predict transfer quality, and that is reported rather than smoothed.
  - **What the pair buys is robustness.** `cpvi_transfer` beats both surface baselines on TraceElephant agent accuracy under **two independent calibrations**, with non-overlapping intervals both times against schema validity 0.263 and mean cosine 0.254, at zero model calls. The A2 calibration is retained as the sensitivity arm and `runs/rq1/8902072e1f47b6de/README.md` is marked superseded-as-transfer-source rather than deleted.
  - `cosine` reads AUROC 0.569 with **orientation −1** on the confirmation against +1 on A2. A near-chance, single-class statistic flipping sign between calibration sets is exactly why orientation is read from the report and never typed as a flag — and why `cosine` is not the transferred arm.
- **`docs/myriad.md` §9a no longer paraphrases a gate threshold.** It had stated G1 as *easy ≥ 8/20 and medium ≥ 3/20*; PREREGISTRATION §6 states *≥ 0.5 at easy difficulty*, easy-only, and `PilotConfig.g1_success_floor` implements the register. The run satisfies both so no verdict turns on it, but at easy 8/20 or 9/20 the two documents would have disagreed about a gate outcome, with the more permissive statement sitting in the runbook the submitter reads. §9a now quotes the register, and PREREGISTRATION records the rule: runbooks cite thresholds, never restate them.
- **`runs/rq1/188a3d556b824e3e/lineage.csv`** marks `86ecbbdf35322dc3` `gate-verdict` with job 236653, `git_sha` 9631852 and the realised result, and pre-registers the E3 re-gate row with both candidate hashes and the decision still open.
- **The two frozen RQ3a results are re-frozen with the transfer arm live (`runs/rq3a/traceelephant/`, `runs/rq3a/who_and_when/`, DSE-024).** Statistic transferred: `fail`, calibrated on the v9 A2 pilot (`8902072e1f47b6de`, n=1013 handoffs, held-out AUROC 0.593, orientation +1). `cosine` is not transferred and could not usefully be: it fits nothing, so the existing `mean_cosine` row already *is* it computed directly on the logs.
  - **TraceElephant (118 evaluable traces): the transferred statistic beats both baselines on every metric.** Agent accuracy **0.576 [0.492, 0.661]** against 0.263 (schema validity) and 0.254 (mean cosine) - non-overlapping intervals; step 0.169 [0.110, 0.246] against 0.017 and 0.093; MRR 0.376 against 0.244 and 0.255. At zero model calls.
  - **Who&When (150 evaluable traces): it does not.** Step 0.180 against mean cosine's 0.273, MRR 0.381 against 0.448, agent tied at 0.367. The split is the substrate-migration argument arriving as a measurement: Who&When's observations are *reconstructed* from preceding messages (`reconstructed_observation` is set on every row it emits), so a statistic that conditions on the observation has degraded conditioning to work with. This is what DSE-041 predicted on observability grounds and is now evidence rather than argument.
  - **These are not yet comparable to the published Who&When numbers.** The 53.5% agent / 14.2% step figures come from LLM-judge procedures, which is exactly the arm still unrun (`judge` and `agreement` remain `null`). DSE-024 stays open for that reason.
- **`RQ3aManifest` version 1 to 2, gaining `transfer_train_dataset_hash` (`experiments/rq3a_run.py`, DSE-024).** `cfg` already records the transfer directory and key; the manifest now also records *which arena dataset* trained the statistic, so it answers "transferred from what?" without resolving a path that may not outlive the run. A v1 artefact is one that predates the transfer arm.
- **`SweepConfig.thinking` makes decode-time reasoning part of dataset identity (`experiments/sweep.py`).** Carried only on `ServingConfig`, which `dataset_hash_for` never reads, the thinking and non-thinking arms of the D26 ablation would have hashed alike and the writer's resume path would have appended both into one directory — the pooling failure `prompt_version` and the simulation digest already exist to prevent. `False` is dropped from the hash payload exactly as `gate=None` is, so no dataset recorded to date re-keys; pinned by a test in both directions.
- **`scripts/myriad/pilot.sh` drives RQ3a too, rather than gaining a sibling jobscript (DSE-066).** The serve/wait/trap machinery is identical for every driver, so the `DRIVER` switch grew a `preceptx-rq3a` branch instead of a second copy: its `--root` is the **corpus** root (`RQ3A_ROOT`, required), not `RUNS_ROOT`, which would have pointed it at an empty tree, and `--judge` is implied because it is the only part of RQ3a that makes a model call — a GPU job without it would hold an A100 to run sklearn.
- **`scripts/myriad/prefetch.sh` pulls the RQ3a corpora when `RQ3A_ROOT` is set.** Same reasoning as the weights and the encoder: ~800 MB downloaded inside a GPU job is allocation spent on network I/O, and dead outright if compute nodes have no outbound route. Pulled on the host branch rather than inside the image, because `curl` and `unzip` are login-node tools the image is not required to carry.
- **The orientation hold now prevents contact rotation instead of reverting it (`sim/actions.py`, DSE-059).** `hold_orientation` restored the pre-action angle *after* the settle, which stops the rotation being **recorded** but not from **happening**: from a 30° start on medium (geometric window ±17.2°) the load rotates itself to 0.48° under contact torque, slips through the channel, and is written back as 30.00°. Two consequences, both live in run 227886 — the DSE-058 degeneracy was hidden rather than closed (medium's realised window was ±32.6°), and the recorded angle was not the angle at which the load passed the gap. Now implemented as an **infinite moment for the duration of a non-rotate action**, which reproduces geometric fit to **0.01°** across seven apertures and is monotone in aperture; restore-after erred by up to 20.03° and was non-monotone. Angle drift over ten pushes is exactly 0.0.
- **The rotation quantum is authored and the impulse derived from it (`sim/actions.py`, DSE-059).** `ROTATION_STEP_DEG = 12.0` with `angular_impulse` computed through a measured constant (`DEG_PER_UNIT_IMPULSE = 115.582`), reversing the direction that caused the bug: the old default was an impulse authored directly for the T's moment of inertia and silently inherited by a bar of 1.71× smaller moment, turning 34°/action into 57.79°. Free rotation is **exactly deterministic** (sd `0.000000` across 37 start angles at seven impulses), so the constant is exact and any spread in a real run is contact truncation. Sizing is a **lattice** argument, not `step < window`: with half-turn symmetry and bidirectional rotation the reachable set is `θ₀ + m·step (mod 180°)`, which is not monotone in the step — 9.25° leaves hard unreachable where 11.67° reaches it.
- **`_ANG_RES` is derived from the rotation step and refuses to load above it (`sim/feasibility.py`, DSE-059).** The planner's pose-dedup bucket was a bare 18°, correct against a 57.8° step and silently wrong the moment the step fell below it — a bucket wider than the step collapses consecutive rotations into one search state, so the planner prunes the poses the threading manoeuvre needs.
- **Start-pose `y_range` scoped to the alignment tolerance (`sim/arena.py`, DSE-059).** It was (1.5, 4.5) — a *continuous* jitter against a **quantised** actuator, the same fault in the positional axis: a flat bar clears the narrowest channel only within 0.12 units of the slit centre, but N/S moves it in a deterministic 1.034-unit quantum, so most starts were uncorrectable. Geometry alone capped success at **77/39/23%** before either agent reasoned about anything. Now ±0.096 (80% of the derived tolerance). The scripted policy solves **32/32** seeds at every rung; it solved 11/7/4 before.
- **Hard aperture 0.50 → 0.64 (`sim/arena.py`, DSE-059).** Its window was 8.26°, reachable only by lattice resonance: of 25 round step angles from 8° to 20°, exactly **three** gave full coverage at 0.50 and several left the rung unreachable at any budget; at 0.64 all 25 do. The rung's difficulty is unchanged on the axis the ladder grades — slack stays **±3 / ±1 / ±0**, so hard still tolerates no miscount. Effective aperture 0.54 remains inside the rotation-required band [0.3, 1.4).
- **Step budgets 20/25/25 → 30/35/35, re-certified (`sim/feasibility.py`, DSE-059).** All three rungs now solve with **plannable** paths — `ROT+ ×5, E ×7` / `ROT+ ×7, E ×8` / `ROT+ ×7, E ×7`. Difficulty grades by rotation-count **slack**, not count: medium and hard both need seven rotations (the previous ladder did not separate them either, at 10 and 10).
- **`collision_slop` pinned and fingerprinted (`sim/arena.py`, DSE-059).** It was pymunk's implicit default at 25% of the hard rung's effective aperture, so a change in the library default would have moved the difficulty ladder silently. Value unchanged; only its provenance is.
- **`ENVIRONMENT_SCHEMA_VERSION` 2 → 3 (`sim/fingerprint.py`).** `hold_orientation` is fingerprinted by *value* and its value did not change — DSE-059 altered what `True` means — so the schema version is what re-keys the dataset. This is exactly the case that field exists for. The module docstring claimed `StepConfig` was not fingerprinted while the code hashed it; corrected.
- **The prompt states the action quanta (`agents/prompts.py`, `PROMPT_VERSION` → v7, DSE-059).** "about 1.03 units" per push and "exactly 12 degrees" per rotation. Hard tolerates no miscount, so leaving the step implicit would make it a constant-*discovery* task and the capability arm would measure the wrong thing; stating it follows standard practice in embodied spatial-reasoning benchmarks. A unit test asserts the stated numbers against the live actuator.
- **`InfoStatistic` retired in favour of `FailStatistic` (`gate/statistics.py`, `gate/calibration.py`, DSE-061).** They rank handoffs identically — Spearman **−1.000000**, exactly, since for a binary outcome the entropy of `g_cond` is a symmetric unimodal function of the probability `FailStatistic` reports — so the gate had two statistics rather than three, and a result "holding for both" held once. `FailStatistic` survives as the one monotone in the quantity the gate is calibrated against. The class is **kept, not deleted**, so pre-DSE-061 artefacts still unpickle, and `resolve_statistic_key` maps `"info"` → `"fail"` with a warning so recorded manifests and configs still load.
- **`docs/experiment_design_log.md` and `docs/methodology.md` §10.5: D25 added.** The first
  register entry written **after a result was read**, and marked as such throughout. Records the
  actuator diagnosis, the four correction paths considered and rejected (re-pointing the frozen
  `Y`; the 32B factorial at full scale; re-tuning `angular_impulse` and re-running *this* grid;
  repairing rather than demoting the H1 mixed model), and fixes that apply only to a future task
  generation with a different `dataset_hash`. **No threshold in §5, §6 or §9.10 moves.** The §10.5
  preamble is amended to admit post-hoc entries as a third and weaker category rather than folding
  D25 in with D23-D24: what bounds D25 is not blindness but scope.
...
### Fixed
- **The embedding encoder returned different vectors for the same string (`measure/featuriser.py`).** `sentence-transformers` auto-selects a backend; on Apple Silicon that is **MPS**, which encodes one repeated text to *substantively different* vectors depending on which batch it lands in. On `torch 2.10.0` / `sentence-transformers 5.6.0` against the pinned `BAAI/bge-base-en-v1.5@a5beb1e3`, a string repeated 64 times across a 32-wide batch boundary read **min cosine 0.542745** to its own first row, 62 of 64 rows below 0.999; the same call on CPU holds 0.999999999999 (≈1.8e-07 elementwise, ordinary float32 jitter). A 0.46 cosine gap is a different vector, not noise.
  - **Blast radius is the whole measurement stack, not one experiment.** The same `Featuriser` feeds `e_s`/`e_m` to the CPVI estimator, the retrospective/prospective twin, `CosineStatistic`, the gate calibration and the G2 pilot gate. Any of those computed from a local analysis on an Apple-Silicon machine is unreliable.
  - **How it surfaced.** A gratuitous re-run of an identical RQ3a command moved a headline number (TraceElephant `mean_cosine` step accuracy 0.093220 → 0.101695). `embed_texts` caches by content hash and writes one vector per unique text, so a *cold* run sees up to 316 different vectors for TraceElephant's most-repeated message while every *warm* run sees the single cached one. Nothing crashed and no status flipped.
  - **Fix, part one:** `EncoderConfig.device`, defaulting to `"cpu"` and passed explicitly to `SentenceTransformer`. A regression test encodes one string across two batch widths and asserts min cosine above `1 - 1e-6` — above CPU's float floor, far below the MPS failure — so it separates jitter from a wrong vector without pinning a float.
  - **Fix, part two: `embed_texts` deduplicates before encoding and fans back out.** Pinning the device left the cold-versus-warm asymmetry itself in place, with a second and much smaller cause that only became visible once the first was gone: two warm runs agreed bit-for-bit while cold-versus-warm still moved MRR in the fourth decimal (0.254741 → 0.254975). CPU's own 1.8e-07 batch jitter reorders near-ties, because a cold run encodes a repeated string once *per occurrence* — each in a different batch slot — while the cache stores one vector for it. Encoding each unique text once makes cold and warm identical by construction on any backend, and is markedly cheaper (TraceElephant: 2,488 message slots → 1,166 encodes).
  - **A latent misalignment closed in the same function.** The old `[v for v in vectors if v is not None]` would have returned *fewer rows than inputs* had any slot gone unfilled, silently shifting every downstream `e_s`/`e_m` pairing. The result is now indexed by text, so alignment holds by construction.
  - **Not covered either way: CUDA.** Job 227886's analysis ran on an A100 on Myriad, a different kernel path. The cluster results are neither implicated nor cleared; the check has not been run there.
  - The local `.embed_cache` (50,975 vectors, a mix of poisoned and correct) is purged, and both RQ3a runs redone.
- **A calibrated gate could name a statistic that no longer exists.** `GateConfig.statistic_key` now resolves retired keys through a validator rather than failing at construction, so configs recorded before DSE-061 load unchanged.
