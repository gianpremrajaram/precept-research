...
- **`experiments.rq1.action_agreement` and `ActionAgreement` — the per-condition form of G3's correctness limb.** The gate pools the cell; the pooled number is what hid the E3 mechanism.
  - **Same instrument, per condition.** Agreement with `sim.feasibility.oracle_action` against a within-episode permutation null on B's own actions, 200 permutations, seeded. Reports `agreement`, `null_mean`, `null_p95`, `p_value`, plus three supporting measures a bare agreement number cannot supply.
  ...
  - **First reading (`af50c7c12d65540f`):** C0 0.330 vs null 0.330 (*p* = 0.070), C1 0.046 (*p* = 0.095), C3 0.261 vs 0.340 (*p* = 1.000), **C4 0.399 vs 0.388 (*p* = 0.010, PASS)**. Only the lossy channel produces a receiver that acts on the pose above chance.
