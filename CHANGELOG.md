...
- **`preceptx-rq3b` and the H6 causal-gate arms (`experiments/rq3b.py`, `experiments/cli.py`, DSE-025).** DSE-018 shipped the gate and its two score-blind controls; this is the experiment that uses them. RQ1 and RQ2 detect — this asks whether detection becomes enforcement.
  ...
  - **`verdict` separates a null from an untestable comparison.** "The gate does not beat its score-blind controls" is a reportable finding about the statistic. Every arm returning *identical* outcomes is not — it means the grid produced no variance to move, which is the live risk after job 232980's 1/96, and is named as a statement about the grid rather than about the gate.
...
- **Prompt v8: the pose-dependent clearance in every state form (`sim/load.py`, `sim/serialise.py`, `agents/prompts.py`, D26).** Job 232980 floored at 1/96 with 97.6% of `E` actions issued at poses that could not fit the slit; 99.9% of messages quoted the angle and 6.6% attempted the projection.
  ...
  - **`serialise.clearance_line`** puts the span in **all three** forms — numeric, grid header and NL. Isomorphism is the reason: withholding it from one form would make that form measure trigonometry rather than representation, which is not the axis the serialisation A/B exists for. It states the span, not a verdict — the slit comparison, the rotate-then-translate ordering, the y-alignment and the two-wall repetition remain the agent's inference.
  ...
  - **C3 windowing keeps the clearance line in the header block.** C3 restricts *visible rows*, not information content; windowing the span out would have made it a different treatment.
  - `PROMPT_VERSION` v7 → v8, so v7 (`188a3d556b824e3e`) stays intact as the baseline arm rather than being resumed into.
