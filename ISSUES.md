...
### Context
TraceElephant (ACL 2026, CC-BY-4.0) records each step as step id, agent id, agent name, `input`, `output` and tool logs, plus trace metadata. **Corrected against the corpus 24 Aug 2026 (E9):** the fields are `input`/`output` (structured OpenAI objects, not the strings `input_context`/`output_content`), and the corpus is **220 traces, every one an annotated failure** - not 380 executions of which 220 are failures. There are no non-failing executions, so the refit arm is undefined here too and DSE-042's replay becomes the only route to a two-class step-level *Y*. See `docs/rq3a_schema_mapping.md`. `input_context` is the receiver-observed state the conditional construct requires; Who&When records outputs only and therefore cannot supply it without reconstruction, and all 184 of its instances are failures, so the refit arm is undefined on it rather than merely difficult. This ticket makes TraceElephant the primary RQ3a substrate.
