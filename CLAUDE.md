...
7. **CHANGELOG.md.** Update the `[Unreleased]` section under the appropriate category (Added, Changed, Deprecated, Removed, Fixed). Every behaviour- or result-affecting change gets an entry. **Write entries granularly — this is a reference doc the team reads back, not a one-line summary.** Per ticket, name each component added and, for each, capture its *key signals*: the load-bearing design decisions, invariants, and concrete parameters someone needs to understand or audit it (e.g. "the T's vertical extent is symmetric about the body so it centres on the slit"; "slit widths 1.8/1.2/1.1 kept above the 1.0 stem-length threading threshold"; "world-frame impulse at the COM with quasi-static settling"). Prefer sub-bullets per component over a dense paragraph; record non-obvious assumptions honoured and notable deviations. Do not retroactively expand already-merged entries — apply this depth from the current ticket forward.

   **Experiment Design Log (`docs/experiment_design_log.md`) — maintained alongside, not instead of, the CHANGELOG.** Whenever a change alters the *experiment or research design* — task geometry, difficulty semantics, the outcome Y or probe family V, conditioning choices, channel/serialisation/gate design, or anything that changes what a result *means* or how a reviewer reads the method — add a dated entry to `docs/experiment_design_log.md` using its template (date · area · trigger · finding · impact · risk reduced · correction path · the fix · result · so-what). This does **not** replace the CHANGELOG entry: the CHANGELOG records *what the code now does* (component-level, behaviour); the design log records *why the design changed and what risk it closed* (research-level, interpretation). A change that touches both gets one entry in each; a routine code change with no design/interpretation impact goes only in the CHANGELOG. Reserve the design log for the larger experiment/research-question-level decisions, not every diff.
...
8. **Always update CHANGELOG.md** for behaviour- or result-affecting changes, under `[Unreleased]`; and **`docs/experiment_design_log.md`** when the change alters the experiment/research design (task geometry, Y/V, conditioning, difficulty, channel/gate design — see Ticket workflow §7). They are complementary, not substitutes.
