...
(`experiments.rq1.action_agreement`, 2,000 within-episode permutations each, seeded per condition):
**only C4's receiver beats its own within-episode null** — agreement 0.399 against a 95th percentile
of 0.388, *p* = 0.004, surviving Bonferroni across the four tests — while C0 sits at 0.330 against
0.331 (*p* = 0.069), C1 at 0.046 and C3 at 0.261 against 0.342. Pooled rotation-direction agreement is
...
- **The gated statistic is `cosine`**, declared here before the `qsub` because §8a requires the
  instrument to be named in writing and not only the direction. It is the probe-independent
  statistic, so a *positive* result could not be dismissed as the probe grading its own homework —
  which is the objection a causal gate result has to survive, and the reason it is preferred to
  `fail` despite `fail`'s higher AUROC. It is gateable only now: on the E3 corpus
  (`af50c7c12d65540f`) `cosine` reads AUROC **0.766** at threshold **−0.647** and firing rate
  **0.200**, against **0.569** — barely above chance — on the C0-only G1 corpus. **If `cosine`
  turns out too weak to fire meaningfully** (an observed firing rate far from the calibrated 0.200
  on the run's own arms), that is reported as an instrument failure and the null is reported as
  uninformative rather than as support for the prediction — the way this declaration can disappoint
  its author.
