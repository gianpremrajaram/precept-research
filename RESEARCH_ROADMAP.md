...
**Substrate.** TraceElephant (ACL 2026, CC-BY-4.0) is primary because it records the receiver's input context as well as the acting component's output per step, which maps onto the observation and the message with no approximation; and it ships runnable environments, which is what makes replay and live interception possible at all. **Corrected 24 August 2026 against the corpus itself (E9, `docs/rq3a_schema_mapping.md`).** Two claims in the original paragraph do not survive contact with the files. The per-step fields are `input` and `output`, structured OpenAI objects rather than the strings `input_context` and `output_content` — a naming matter only; the conditional construct is intact. But "380 executions of which 220 are annotated failures, so trace-level outcome is genuinely two-class" is wrong: the corpus is **220 traces in total, every one an annotated failure**, with no non-failing executions at all. Only 44 traces (the `swe-agent` family) carry an annotation-free outcome, and 0 of those 44 pass. Trace-level outcome on the primary corpus is therefore **single-class**, and the refit arm is undefined on it for the same reason it is undefined on Who&When. The substrate move is still right — the receiver-observed context is the reason that mattered, and it is real — but it was argued on two grounds and only one of them held. The consequence is that counterfactual replay (DSE-042) is load-bearing rather than an upgrade path: it is the only route to a within-trace two-class *Y* on a corpus that also records the conditioning state. Who&When is retained as a transfer-only comparability anchor so the familiar published numbers sit in the same table, with every row it emits carrying an explicit reconstructed-observation flag so approximated and true conditioning state can never be silently mixed. MAST-Data is a cheap secondary for the coarse trace-level question only; its annotations are trace-level and cannot test step localisation.
...
| 1. Pilot | Task viability and presence of an information gradient established | Simulator, agent loop, conditions C0-C4, episode runner, logging | G1/G2/G3 pass, or invoke fallback ladder | **Closed `fallback`, 29 Aug 2026** (`af50c7c12d65540f`). G1 PASS 0.800, G2 FAIL −0.250, G3 groundedness PASS 1.000, G3 correctness FAIL 0.242. Ledger spent; no attempt 3. Rung 3 is the finding, with the receiver-competence mechanism |
| 2. Measurement stack | Y definition and probe family locked; CPVI estimator validated on pilot data | Featuriser, PVI/CPVI estimator, probe training, divergence proxy, runtime statistics | **Y and V frozen; encoder pinned; selectivity and repeat count set** | **Built and validated.** Encoder pinned (`bge-base-en-v1.5`@`a5beb1e`); DSE-043/044 landed and the length control fired on E3. F0 will not fire — see PREREGISTRATION §8a |
| 3. RQ1 main runs | ~~H1 gradient~~ → **H1 rejected with a mechanism; H2 supported** | Full factorial sweep executed and logged; self-play + heterogeneous + serialisation cells | ~~RQ1 result frozen~~ → **F1′ RQ1 (negative) frozen for write-up** | **E4 will not run** — its precondition failed. RQ1 is written up as rung 3: the absent gradient as the finding, with the per-condition receiver-competence mechanism. Data in hand |
| 4. RQ2 + gate calibration | H3/H4 result: twins agree, proxy tracks CPVI; gate operating point chosen | Twin and proxy analysis; calibrated gate threshold | Gate ready for RQ3b | **Analysed 30 Aug 2026 on the E3 corpus.** H4 supported on all three statistics (corrected ρ 0.269–0.275, all excluding zero; `fail` AUROC 0.906); encoder-invariant; selects `y_terminal_success` as RQ3b's target. **E7 calibration run and frozen** (`runs/rq1/af50c7c12d65540f/calibration.json`): `fail` thr 0.977, `cosine` thr −0.647, firing rate 0.200 at n = 3,419 — R5 imports it, never re-derives it. Awaiting F2 freeze |
| 5. RQ3b causal arm | H6 result: gating beats both controls, or a clean null | Gate integration, retry feedback template, gate-active runs + both controls | Causal claim frozen | **Built** — `gate/{integration,controls,statistics,calibration}.py`, `experiments/rq3b.py`, `preceptx-rq3b`, all four modes. **Not run.** Direction pre-declared in PREREGISTRATION §8c: a null is predicted, and predicted |
| 6. RQ3a external validity (parallel) | H5 result: CPVI localises real failures on the operating-characteristic axis | TraceElephant loader, replay labeller, localisation analysis under both regimes | External-validity claim frozen | **Built and the transfer regime is frozen** (`runs/rq3a/*`). TraceElephant CPVI transfer 0.492 [0.407, 0.585] vs baselines 0.263 / 0.254 (amendment A5, re-keyed to the E3 calibration 30 Aug; 0.525 pre-A5); Who&When a nominal loss on overlapping intervals. **The refit regime is undefined without E10** (both corpora single-class) and the judge arm is unrun |
...
**The honest read on where the project sits (revised 30 Aug 2026).** The engineering is done, the science has run, and the headline has moved. Every phase's code is merged and tested; four Myriad datasets exist; the E3 ledger is closed at `fallback`. **RQ1's H1 is rejected and the rejection has a named mechanism** — only the lossy channel produces a receiver whose actions beat their own within-episode permutation null, so there is no gradient to measure in a pair that never reads the state. **RQ2 and RQ3a are the positive results**, and RQ2 passes on the very corpus where H1 fails. What remains is not building: it is freezing three results that already have their data, and spending queue time on RQ3b (direction pre-declared), the RQ3a judge arm, and the declared post-hoc cells. The constraint is now write-up throughput, and the plan should treat it as one.
